...
function mostrarBusquedaProfesores() {
    ...
    results.innerHTML = '';
    input.value = '';
    profesoresSeleccionadosBusqueda = new Set(); // Limpiar selección previa al entrar
    // No mostrar ningún docente al abrir, solo la barra vacía
}

// NUEVO: Filtrar profesores por nombre
function filtrarProfesoresPorNombre(nombre) {
    const profesoresSet = new Set();
    Object.values(profesoresPorGrupo).forEach(arr => arr.forEach(p => profesoresSet.add(p)));
    const profesores = Array.from(profesoresSet);
    return profesores.filter(p => p.toLowerCase().includes(nombre.toLowerCase()));
}
...
function renderizarResultadosBusqueda(profesores) {
    ...
    // Mostrar solo coincidencias de búsqueda o los ya seleccionados
    // Si el input está vacío, solo mostrar los seleccionados (si hay)
    const input = document.getElementById('search-teacher-input');
    const searchValue = input.value.trim();

    // Mostrar coincidencias si hay búsqueda
    ...
    // Mostrar los seleccionados aunque no sean coincidencia de búsqueda
    if (searchValue.length === 0 && profesoresSeleccionadosBusqueda.size > 0) {
        profesoresSeleccionadosBusqueda.forEach(profesor => {
            const checkboxId = `search-teacher-${profesor.replace(/\s+/g, '-')}`;
            const html = `
                <label class="checkbox-option">
                    <input type="checkbox" name="search-teacher" value="${profesor}" id="${checkboxId}" checked>
    ...
    Array.from(results.querySelectorAll('input[type="checkbox"][name="search-teacher"]')).forEach(cb => {
        cb.addEventListener('change', (e) => {
            if (e.target.checked) {
                ...
                // Si se deselecciona y no hay búsqueda, quitar el docente de la lista mostrada
                if (searchValue.length === 0) {
...
function configurarEventListeners(){
  ...
  document.getElementById('back-to-teachers').addEventListener('click', () => {
    ...
    if (desdeBusquedaDocente) {
      document.getElementById('search-teachers-section').classList.remove('hidden');
      renderizarResultadosBusqueda([]); // Muestra los seleccionados
      desdeBusquedaDocente = false; // Solo regresa una vez
    } else {
      document.getElementById('teachers-section').classList.remove('hidden');
    }
  });
  ...
  // NUEVO: Ir a la sección de búsqueda de profesores
  document.getElementById('go-to-search-teachers').addEventListener('click', () => {
    mostrarBusquedaProfesores();
    document.getElementById('semesters-section').classList.add('hidden');
    document.getElementById('search-teachers-section').classList.remove('hidden');
    renderizarResultadosBusqueda([]);
    desdeBusquedaDocente = false; // Aún no está en evaluación
  });

  // NUEVO: Buscar profesores al escribir
  document.getElementById('search-teacher-input').addEventListener('input', (e) => {
    const nombre = e.target.value;
    renderizarResultadosBusqueda(filtrarProfesoresPorNombre(nombre));
  });

  // NUEVO: Volver de la búsqueda a los semestres
  document.getElementById('back-to-semesters-from-search').addEventListener('click', () => {
    document.getElementById('search-teachers-section').classList.add('hidden');
    document.getElementById('semesters-section').classList.remove('hidden');
  });

  // NUEVO: Confirmar selección de profesores desde búsqueda
  document.getElementById('confirm-search-teachers').addEventListener('click', function () {
    // Asegura que los checkboxes actuales actualicen el set antes de continuar
    const checkboxes = document.querySelectorAll('input[name="search-teacher"]');
    ...
    if (seleccionados.length === 0) {
      alert('Selecciona al menos un docente para calificar.');
      return;
    }
    ...
    if (crearEvaluaciones()) {
      document.getElementById('search-teachers-section').classList.add('hidden');
      document.getElementById('evaluation-section').classList.remove('hidden');
      document.getElementById('search-teacher-input').value = '';
      document.getElementById('search-teacher-results').innerHTML = '';
      desdeBusquedaDocente = true; // Marca que viene de búsqueda
    }
  });
